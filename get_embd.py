...
def get_args():
    ...
    parser.add_argument('--model_path', type=str, default='pretrained_model/best-m-1006000', help='model path')
    parser.add_argument('--read_path', type=str, default='face.jpg', help='path to image file or directory to images')
...
def load_image(path, image_size):
    ...
    for path in paths:
        ...
        img = imread(path)
        img = np.array(Image.fromarray(img).resize((image_size, image_size), Image.BILINEAR)).astype(np.double)
...
if __name__ == '__main__':
    ...
    if args.mode == 'build':
        ...
        tf_config.gpu_options.allow_growth = 0.4
